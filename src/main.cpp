...
int main(int argc, char* argv[])
{
    ...
    //Print results
    float tot_num_ops_est = 7 * n + 3 * num_edges + 5 * n * log(n) / log(2);
    ...
    std::cout << "estimated number of operations 7V + 3E + 5VlgV: " << tot_num_ops_est << std::endl;
    std::cout << "ratio complexities: " << (tot_num_ops / tot_num_ops_est) << std::endl;
    std::cout << "done" << std::endl;

    //Deallocate tree reference
    free_node_ref(min_span_props.node_arr, n);
